(() => {
    ...
    function renderStrategyDetails(strategyKey, target) {
        ...
        if (!strategy) return;
        const chipClass = {
            yield: 'chip-yield', static: 'chip-static',
            dynamic: 'chip-dynamic', joint: 'chip-joint'
        }[strategyKey] || '';

        target.innerHTML = `
      <div class="strategy-meta">
        <span class="strategy-chip ${chipClass}">${strategy.badge}</span>
        <span class="strategy-chip ${chipClass}">${strategy.ius}</span>
      </div>
      <h3 class="title is-5">${strategy.label}</h3>
      <p style="margin-bottom:0.6rem;">${strategy.description}</p>
      <p><strong>Observed usage:</strong> ${strategy.usage}</p>
      <p class="has-text-grey" style="margin-top:0.4rem;"><em>${strategy.note}</em></p>
    `;
    }
    ...
    function drawAgent(layout, r, c, color, label) {
        ...
        ctx.fillStyle = "#ffffff";
        ctx.font = `bold ${Math.max(12, radius)}px "Google Sans", sans-serif`;
    ...
    function drawConflict(layout, r, c) {
        ...
        ctx.font = `${Math.max(12, cell * 0.4)}px "Google Sans", sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
    ...
    function markCell(layout, r, c, color, label) {
        ...
        if (label) {
            ctx.fillStyle = color;
            ctx.font = `${Math.max(10, cell * 0.25)}px "Google Sans", sans-serif`;
            ctx.textAlign = "center";
